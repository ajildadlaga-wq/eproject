...
const isOverdue = (t: Task) => t.status !== "DONE" && !!t.planned_end && t.planned_end < todayStr;
      ...
      <form onSubmit={submit} className="grid gap-3 md:grid-cols-6">
        ...
        <div className="md:col-span-2"><label className="label">{t("c.status")}</label>
          <select className="input" value={status} onChange={(e) => setStatus(e.target.value as TaskStatus)}>
            {STATUSES.map((s) => <option key={s} value={s}>{t("tstatus." + s)}</option>)}
          </select></div>
...
function ProgressDialog({ projectId, task, requireReport, onClose }: {
  ...
  const save = useMutation({
    mutationFn: () => {
      const status: TaskStatus = progress >= 100 ? "DONE" : progress > 0 ? "IN_PROGRESS" : task.status;
...
// ---- Row ------------------------------------------------------------------
function Row({ t, projectId, canEdit, canDelete, onEdit, onProgress }: {
  t: Task; projectId: string; canEdit: boolean; canDelete: boolean;
  onEdit: (t: Task) => void; onProgress: (t: Task) => void;
}) {
  const qc = useQueryClient();
  const toast = useToast();
  ...
  return (
    <tr>
      <td className="font-medium text-slate-800 dark:text-slate-100">{t.name}</td>
      <td>
      ...
      <td>
        {canEdit ? (
          <select className={selectCls} value={t.status} disabled={patch.isPending}
            ...
            {STATUSES.map((s) => <option key={s} value={s}>{tr("tstatus." + s)}</option>)}
          </select>
        ) : <span className="text-slate-500">{tr("tstatus." + t.status)}</span>}
      </td>
      <td className="text-right whitespace-nowrap">
        {canEdit && <button className="btn-ghost mr-1 px-2 py-1 text-xs text-emerald-700" onClick={() => onProgress(t)}>{tr("c.update")}</button>}
        {canEdit && <button className="btn-ghost mr-1 px-2 py-1 text-xs" onClick={() => onEdit(t)}>{tr("c.edit")}</button>}
        {canDelete && (
...
export default function TasksTab({ projectId, canEdit }: { projectId: string; canEdit: boolean }) {
  ...
  const requireReport = role === "EDITOR";
            ...
            return (
              <div key={g.status} className="card-table">
                <div className="flex items-center gap-2 px-4 py-3">
                  <span className={`badge ${g.pill}`}><span className={`h-1.5 w-1.5 rounded-full ${g.dot}`} />{t("group." + g.status)}</span>
                  ...
                  <tbody>
                    {rows.map((t) => (
                      <Row key={t.id} t={t} projectId={projectId} canEdit={canEdit} canDelete={canDelete}
                        onEdit={setDialog} onProgress={setProgressTask} />
                    ))}
                  </tbody>
                </table>
              </div>
